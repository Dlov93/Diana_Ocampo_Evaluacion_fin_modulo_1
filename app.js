...
const URL_API = 'https://script.google.com/macros/s/AKfycbw19b23g5qWXYu4UCxZ5lFpJ9uvzu3NbuYtD760CSZHX8YRGZXU1e_AZ87_JY3PiKq6tg/exec'
...
async function enviarMultiplesMensajes(mensajes) {
  for (const mensaje of mensajes) { // completar esta línea
...
async function obtenerUltimoMensaje() {
  ...
  const datos = await respuesta.json();
  const ultimo = datos[datos.length -1]; // completar esta línea
...
module.exports = { enviarMensaje, enviarMultiplesMensajes, obtenerTodosLosMensajes, obtenerUltimoMensaje };